#!/usr/bin/env python3
"""Test script for LlamaCpp provider (Step 5 checklist)."""
...
try:
    from langextract_llamacpp import LlamaCppLanguageModel
except ImportError:
...
print("Testing LlamaCpp Provider - Step 5 Checklist:")
print("-" * 50)
...
try:
    ...
    schema = LlamaCppSchema.from_examples(examples)
    print(f"   ✓ Schema created (keys={list(schema.schema_dict.keys())})")

    schema_class = LlamaCppLanguageModel.get_schema_class()
    print(f"   ✓ Provider schema class: {schema_class.__name__}")

    provider = LlamaCppLanguageModel(model_id=_example_id(PATTERNS[0]) if PATTERNS else "test-model")
